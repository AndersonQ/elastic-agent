		...
		t.Run(name, func(t *testing.T) {
			for _, tc := range tcs {
				t.Run(tc.name, func(t *testing.T) {
					...
					if len(tc.wantComps) > 0 &&
						len(tc.wantComps[0].Units) > 0 &&
						comps[0].Units[0].Config != nil &&
						comps[0].Units[0].Config.Source != nil {

						unitCgf := comps[0].Units[0].Config.Source.AsMap()
						wantUnitCfg := tc.wantComps[0].Units[0].Config.Source.AsMap()
